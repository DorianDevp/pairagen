...
pub(crate) fn generic_prompt(req: &BackendRequest) -> String {
    let value = json!({
            ...
            "rules": [
                ...
                "If limits.goal_completion is true, prepare the complete change for the supplied buffer in one patch. Use up to limits.hunks_per_patch hunks and limits.changed_lines per hunk; Pair reviews them locally. Set goal_complete true when accepting the patch finishes the goal, false only when another file or independently inspected stage remains.",
