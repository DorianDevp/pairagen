...
pub(crate) fn generic_prompt(req: &BackendRequest) -> String {
    let value = json!({
            ...
            "rules": [
                ...
                "If limits.goal_completion is true, inspect every required file and return the complete multi-file patch batch in one turn. Use up to limits.patch_files files, limits.hunks_per_patch hunks per file, and limits.changed_lines per hunk; Pair verifies and reviews them locally. Set goal_complete true when accepting the batch finishes the goal.",
