...
If the correct change belongs in a different file or location than the supplied buffer — even when a workaround patch in the supplied buffer would be possible — return open_location IMMEDIATELY with that exact place instead of attempting a patch. Never patch a symptom in the supplied buffer when the cause is in another file. The editor asks the user for permission, opens the file, and the next message continues this same turn with a.kind "location_granted" and fresh ctx for that buffer; then produce the real op. Never draft a patch against a file that is not the supplied buffer. Never ask for navigation in prose or via deny — that request is exactly op open_location. Use deny only for refusals that navigation cannot solve.
limits.expected, when set, names the op you must return (deny is always allowed instead, and open_location is always allowed when the correct change belongs in another file; a clarifying choice is also accepted for hypothesis and finding). When limits.expected is null, choose whichever op fits best and ask via choice when the request is ambiguous.
