...
impl CodexAppBackend {
    pub fn from_env() -> Result<Self> {
        ...
        let args = args_from_env("LOOPBIOTIC_CODEX_ARGS_JSON", "LOOPBIOTIC_CODEX_ARGS")?;
        let model = std::env::var("LOOPBIOTIC_CODEX_MODEL")
            .ok()
            .filter(|value| !value.trim().is_empty());
        let effort = std::env::var("LOOPBIOTIC_CODEX_EFFORT")
            .ok()
            .filter(|value| !value.trim().is_empty())
