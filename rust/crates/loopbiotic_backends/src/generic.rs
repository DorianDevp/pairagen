...
const STRUCTURED_API_CONTRACT: &str = "Return one JSON Loopbiotic op only. No prose. Ops: hypothesis(title,claim,evidence,next,flow_path), finding(title,finding,location,annotation,flow_path), patch(title,explanation,goal_complete,plan,patches,file_ops), choice(title,question,options), deny(title,reason,location), open_location(reason,location), summary(title,summary,changed_files), error(title,message). flow_path is an ordered array of node ids from ctx.call_hierarchy and is empty unless the answer presents a code path. choice.options items are {id,label,action} objects; action is one of follow|why|fix|goal|other_lead|retry|edit_prompt|open|run_check|stop. Use deny when you cannot or should not proceed. When limits.conversation_only is true, never return patch or summary. The user-selected mode and limits.expected define the response contract; never infer or replace the mode. Goal turns are explicitly user-authorized. error is only for technical failures. limits.expected, when set, is the required op. A patch item contains id, file, explanation, and hunks. Each hunk contains 1-based old_start, 1-based new_start, and ordered lines. Each line is {kind,text}, where kind is context, remove, or add and text has no diff prefix or trailing newline. Never write a raw unified diff; Rust renders it from the typed lines. To move or rename files or directories (for example when the context is a directory listing), return a patch op whose file_ops is an array of {kind:\"move\",from,to} with workspace-relative paths and patches []; never mix file_ops and patches in one op, and never express a move as a diff. The editor reviews and applies file_ops behind the same Accept gate; propose content fix-ups (imports) as the next goal step. Unused schema fields null.";
