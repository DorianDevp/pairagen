...
function M.check()
  ...
  local version = require("pair.version")
  ...
  if explicit and explicit ~= "" then
    if vim.fn.executable(explicit) == 1 then
      ok("paird command: " .. explicit)
    else
      error("Configured paird command is not executable: " .. explicit)
    end
  ...
  for _, command in ipairs({ "curl", "tar" }) do
    if vim.fn.executable(command) == 1 then
      ...
      error(command .. " is required for automatic paird installation")
    end
  end
  if vim.fn.executable("sha256sum") == 1 or vim.fn.executable("shasum") == 1 then
    ok("SHA-256 verification is available")
  else
    error("sha256sum or shasum is required for automatic paird installation")
  end
