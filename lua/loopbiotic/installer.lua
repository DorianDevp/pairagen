...
function M.artifact(target)
  return string.format("loopbioticd-v%s-%s.tar.gz", M.version(), target)
end

function M.install_dir(target)
  return string.format("%s/loopbiotic/bin/v%s/%s", vim.fn.stdpath("data"), M.version(), target)
end

function M.install_path(target)
  return M.install_dir(target) .. "/loopbioticd"
end
...
function M.resolve(callback)
  ...
  if distribution.auto_install == false then
    if vim.fn.executable("loopbioticd") == 1 then
      callback("loopbioticd")
    else
      callback(nil, "loopbioticd is not installed and automatic installation is disabled")
    end
    return
  end
  if
  ...
  then
    if vim.fn.executable("loopbioticd") == 1 then
      ...
      callback(nil, "set distribution.repository to an owner/repository or configure backend.command")
    end
    return
  end
  ...
  M.installing = true
  vim.notify("Installing loopbioticd v" .. M.version(), vim.log.levels.INFO, { title = "Loopbiotic" })
...
function M.download(target, callback)
  if vim.fn.executable("curl") ~= 1 then
    callback(nil, "curl is required to install loopbioticd")
    return
  end
  if vim.fn.executable("tar") ~= 1 then
    callback(nil, "tar is required to install loopbioticd")
    return
  end
  ...
  local temporary = string.format("%s/loopbiotic-download-%s", vim.fn.stdpath("cache"), vim.fn.getpid())
    ...
    M.run({
      ...
      M.checksum(archive, function(actual, checksum_error)
        ...
        if actual:lower() ~= expected:lower() then
          callback(nil, "loopbioticd checksum mismatch")
          return
        end
        ...
        M.run({ "tar", "-xzf", archive, "-C", directory, "loopbioticd" }, function(extracted)
          ...
          if not M.executable(path) then
            callback(nil, "installed loopbioticd is not executable: " .. path)
            return
          end
          callback(path)
        end)
...
function M.checksum(path, callback)
  ...
  elseif vim.fn.executable("shasum") == 1 then
    ...
    callback(nil, "sha256sum or shasum is required to verify loopbioticd")
    return
  end
  M.run(command, function(result)
    if result.code ~= 0 then
      callback(nil, "could not verify loopbioticd: " .. M.error_text(result))
      return
    end
